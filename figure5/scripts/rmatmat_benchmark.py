...
def grg_matmat(grg_path, n_vectors):
    ...
    y = np.ones([n_vectors, grg.num_samples])
    dp_time, dp_mem = benchmark(pygrgl.matmul, grg, y, pygrgl.TraversalDirection.UP)
...
def linarg_matmat(linarg_path, block_name, n_vectors):
    ...
    y = np.ones([n_vectors, linarg.shape[0]])
    dp_time, dp_mem = benchmark(lambda: y @ linarg)
    return load_time, load_mem, dp_time, dp_mem
...
def scipy_matmat(genotypes_path, n_vectors):
    ...
    y = np.ones([n_vectors, genotypes.shape[0]])
    dp_time, dp_mem = benchmark(lambda: y @ genotypes)
    return load_time, load_mem, dp_time, dp_mem
...
if __name__ == "__main__":
    ...
    df.write_csv("linarg_grg_scipy_matmat_serial_benchmark.csv")